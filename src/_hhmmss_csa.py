...
for c in connections[begin:]:
    ...
    if sl[parent_stations[deps]] != None and isb4(sl[parent_stations[deps]][3], dept):
        if sl[parent_stations[arrs]] == None:
            sl[parent_stations[arrs]] = c
        elif parent_stations[arrs] != parent_stations[dep_stop] and isb4(arrt, sl[parent_stations[arrs]][3]): # ou isb4(arrt, sl[parent_stations[arrs]][1]) ?
            # c'est mieux !
            sl[parent_stations[arrs]] = c
            for s in neighbours[parent_stations[arrs]]:
                foot_arrt = addtime(c[3], h2hms(walk_time(geo_stations[parent_stations[arrs]], geo_stations[parent_stations[s]])))
                ...
                elif parent_stations[s] != parent_stations[dep_stop] and isb4(foot_arrt, sl[parent_stations[s]][3]):
                    sl[parent_stations[s]] = conn
...
for c in reversed(route):
    ...
    if curr_route != route_or_type and curr_route != "":
        ...
        print("\t" + stopstr(beg_stop), "@", beg_time, "->", stopstr(arrs))
        beg_stop = arrs
        beg_time = arrt
...
print("total time:", timediff(dep_time, route[0][3]))
print("transit time:", timediff(route[-1][1], route[0][3]))
